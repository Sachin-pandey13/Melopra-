...
// layouts (these already live in /layouts)
...
const resolveGenre = (song) => {
  if (!song) return "english";

  const t = (song.title || "").toLowerCase();
  const a = (song.artist || "").toLowerCase();
  const lang = (song.language || song.lang || "").toLowerCase();

  // 1️⃣ If song already has language stored (BEST SOURCE)
  if (lang.includes("bhajan") || lang.includes("devotional")) return "bhajan";
  if (lang.includes("punjabi")) return "punjabi";
  if (lang.includes("hindi")) return "hindi";
  if (lang.includes("english")) return "english";

  // 2️⃣ Bhajan / Devotional
  if (
    t.includes("bhajan") ||
    t.includes("aarti") ||
    t.includes("chalisa") ||
  ...
  // 3️⃣ Punjabi detection
  if (
    a.includes("karan aujla") ||
    a.includes("sidhu") ||
    a.includes("moosewala") ||
    a.includes("diljit") ||
    a.includes("ap dhillon") ||
    a.includes("guru randhawa") ||
    a.includes("shubh") ||
    a.includes("parmish verma") ||
    a.includes("b praak") ||
    a.includes("ammy virk") ||
    a.includes("speed records") ||
    a.includes("white hill music") ||
    a.includes("geet mp3") ||
    t.includes("punjabi") ||
    t.includes("bhangra")
  ) {
  ...
  // 4️⃣ Hindi detection (THIS WAS MISSING IN YOUR CODE)
  if (
    a.includes("arijit") ||
    a.includes("shreya") ||
    a.includes("kk") ||
    a.includes("jubin") ||
    a.includes("atif") ||
    a.includes("neha kakkar") ||
    a.includes("sonu nigam") ||
    a.includes("udit") ||
    a.includes("alka") ||
    a.includes("t-series") ||
    a.includes("zee music") ||
    a.includes("sony music india") ||
    a.includes("saregama") ||
    a.includes("tips") ||
    a.includes("yrf") ||
    a.includes("wave music") ||      // bhojpuri
    a.includes("rehaan records") ||  // punjabi/hindi mix
    t.includes("bollywood") ||
    t.includes("hindi") ||
    t.includes("romantic song") ||
    t.includes("sad song")
  ) {
    return "hindi";
  }
 ...
 if (enqueueOnly) {
  const normalizedQueued = {
    ...
    image:
      ...
      null,
    audio: album.audio || album.streamUrl || "",
    channelId:
      album.channelId ||
...
// ✅ detect genre/lang properly ONCE
const detectedLang =
  (album.lang || album.language || album.genre || "").toLowerCase() ||
  resolveGenre(normalizedAlbum);

// ✅ attach final metadata
normalizedAlbum.lang = detectedLang;
normalizedAlbum.language = detectedLang;
normalizedAlbum.genre = detectedLang;

console.log("🎯 Detected genre/lang:", detectedLang, "|", normalizedAlbum.title);
  ...
  // ✅ Cache recently played songs for offline fallback
try {
  const cached = JSON.parse(localStorage.getItem("melopra_cached_songs") || "[]");

  const updated = [
    normalizedAlbum,
    ...cached.filter((s) => s?.id !== normalizedAlbum.id),
  ].slice(0, 30);

  localStorage.setItem("melopra_cached_songs", JSON.stringify(updated));
} catch (e) {
  console.warn("Failed caching songs", e);
}
...
setPlayedThisSession((prev) => {
  ...
  const ytId = extractYouTubeId(normalizedAlbum);
...
if ((normalizedAlbum.category || "").toLowerCase() === "youtube") {
...
// ---- AUTOPLAY QUEUE GENERATOR (PRODUCTION SAFE + GENRE LOCKED) ----
// ---- AUTOPLAY QUEUE GENERATOR (PRODUCTION SAFE + GENRE LOCKED + OFFLINE CACHE ONLY) ----
const generateAutoplayQueue = async (seedAlbum) => {
  if (!seedAlbum) return [];

  const isOffline = !navigator.onLine;

  const seedId = extractYouTubeId(seedAlbum) || seedAlbum.id;
  const seedGenre = resolveGenre(seedAlbum);

  console.log("🎯 generateAutoplayQueue called:", {
    seedId,
    seedGenre,
    offline: isOffline,
    seedTitle: seedAlbum.title,
  });

  const normalize = (song) => {
    if (!song) return null;

    const sid = extractYouTubeId(song) || song.id;
    ...
    return {
      id: song.id || sid || `auto-${Date.now()}-${Math.random()}`,
      title: song.title || song.name || "Unknown Title",
      artist: song.artist || song.channelTitle || "Unknown Artist",
      image: song.image || song.thumbnail || song.thumbnails?.high?.url || null,
      audio:
        song.audio ||
        song.streamUrl ||
        (sid ? `https://www.youtube.com/watch?v=${sid}` : ""),
  ...
  const filterPlayed = (songs) => {
    ...
    for (const s of songs || []) {
      ...
      const sid = extractYouTubeId(n) || n.id;

      // skip invalid / seed / repeated
      if (!sid || sid === seedId) continue;
      if (playedThisSession.has(n.id)) continue;
      if (playedThisSession.has(sid)) continue;
      if (seen.has(sid)) continue;

      // 🔒 STRICT LANGUAGE LOCK (must match seed)
      const songLang = (n.lang || n.genre || "").toLowerCase();
      if (songLang && songLang !== seedGenre) continue;
  ...
  /* ======================================================
     STEP 0: OFFLINE MODE (ONLY CACHE ALLOWED HERE)
     ====================================================== */
  if (isOffline) {
    console.warn("📴 OFFLINE MODE: Using cached songs only");
    ...
    try {
      ...
      console.log("📴 Offline autoplay from cache:", filtered.length);

      return filtered;
    } catch (err) {
      console.warn("⚠️ Failed to load offline cache:", err);
      return [];
    }
  }

  /* ======================================================
     STEP 1: ML Recommendations (Pinecone) - ONLINE ONLY
     ====================================================== */
  if (currentUser?.uid) {
    try {
      console.log("🧠 Trying ML recommendations (Pinecone)...");

      const g = seedGenre;

      const mlRecs = await getRecommendations(currentUser.uid, {
        id: seedAlbum.id,
      ...
      console.log("🧠 Raw ML response:", mlRecs);

      const filtered = filterPlayed(mlRecs);

      console.log("🧠 Filtered ML autoplay:", filtered);

      if (filtered.length > 0) {
        console.log("✅ ML autoplay USED");
        return filtered.slice(0, 10);
      }

      console.warn("⚠️ ML returned empty (after filtering).");
    } catch (err) {
      console.warn("❌ ML autoplay failed:", err);
    }
  }

  /* ======================================================
     STEP 2: YouTube fallback (ONLINE ONLY)
     ====================================================== */
  try {
    const vid = extractYouTubeId(seedAlbum);

    if (vid) {
      console.log("🎬 Fallback autoplay: YouTube related videos for", vid);

      const related = await fetchYouTubeNext(vid);

      const filtered = filterPlayed(related);

      if (filtered.length > 0) {
        console.log("🎬 YouTube fallback autoplay USED:", filtered.length);
        return filtered.slice(0, 10);
      }
    }
  } catch (err) {
    console.warn("⚠️ YouTube fallback autoplay failed:", err);
  }
...
const updateRecentlyPlayed = (album) => {
 ...
 updateInterestWeights(album, "replay");
};
...
useEffect(() => {
  ...
  // 🧠 ML EVENT: COMPLETE (Local)
  if (selectedAlbum) {
    const ytId = extractYouTubeId(selectedAlbum);

    logUserEvent({
      userId: currentUser?.uid || "guest_user",
      songId: ytId || selectedAlbum.id,
      event: "complete",
...
}, [playNext, selectedAlbum, currentUser]);
...
/>
 ...
 onEnd={() => {
  ...
  const yt = youtubePlayerRef.current;
  const played = yt?.getCurrentTime?.() || 0;
