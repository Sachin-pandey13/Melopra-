 ...
 * - Trending (YT + Deezer) — localStorage cached 30 min
 * - New Releases (Followed artists)
 * - Smart Mixes (Mood/Genre based)
 * - Podcasts — localStorage cached 30 min, deferred
 */
export default function useHomeContent({ library, actions }) {
  // Seed state from cache so first render shows content instantly
  const [trending,    setTrending]    = useState(() => lsRead("hc_trending") || []);
  const [newReleases, setNewReleases] = useState([]);
  const [podcasts,    setPodcasts]    = useState(() => lsRead("hc_podcasts") || []);
  const [loading,     setLoading]     = useState(!lsRead("hc_trending")); // skip loading if cached

  const followedArtists = library?.followedArtists || [];
  const hasFetched = useRef({ trending: !!lsRead("hc_trending"), podcasts: !!lsRead("hc_podcasts") });
  ...
  // 2. TRENDING: YT Music Charts — cached 30 min
  useEffect(() => {
    if (hasFetched.current.trending) return; // already seeded from cache above
  ...
  // 2.5. PODCASTS: deferred 3 s, cached 30 min
  useEffect(() => {
    if (hasFetched.current.podcasts) return; // already seeded from cache
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        lsWrite("hc_podcasts", curated);
        setPodcasts(curated);
      } catch (e) {
        console.error("Home podcasts fetch failed", e);
      }
    }, 3000); // low priority — defer 3 s
    ...
    async function fetchNew() {
      ...
      for (const artist of subset) {
        try {
          ...
          if (yt?.length) {
            allResults = [...allResults, ...yt.slice(0, 4).map((s) => ({ ...s, tag: "New from " + artist.name }))];
          }
        } catch (e) {
      ...
      setNewReleases(allResults.sort(() => 0.5 - Math.random()));
      setLoading(false);
    }
    ...
    smartMixes: [
      {
        ...
        onPlay: async () => {
          ...
          if (songs?.length) { playItem(songs[0], { keepQueue: false }); replaceQueue(songs.slice(1), "ml"); }
        },
      },
      {
